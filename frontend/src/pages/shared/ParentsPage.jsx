    ...
    <DashboardLayout>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Create parent accounts and link them to students.</p>
        </div>
      ...
      <Card>{loading ? <Spinner /> : (
        <Table columns={[
          ...
          { key: 'status', label: 'Status', render: (r) => <Badge status={r.status} /> },
          {
            key: 'actions',
            label: 'Actions',
            render: (r) => <Button size="sm" variant="secondary" onClick={() => openLink(r)}>Link Student</Button>,
          },
        ]} data={parents} />
      )}</Card>

      <Modal open={modal} onClose={() => setModal(false)} title="Add Parent">
        ...
        <Button onClick={handleCreate} className="w-full mt-4">Create Parent</Button>
      </Modal>
